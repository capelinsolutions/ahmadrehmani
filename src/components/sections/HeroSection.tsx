...
const HeroSection = () => (
  ...
  >
    ...
    <div className="absolute inset-0 z-0">
      ...
      />
      {/* Light wash so the clinical scene still shows through */}
      <div className="absolute inset-0 bg-background/40" />
      {/* Centered radial spotlight to make the headline & CTAs pop */}
      <div
        className="absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse 70% 60% at 50% 50%, hsl(var(--background) / 0.92) 0%, hsl(var(--background) / 0.75) 35%, hsl(var(--background) / 0.25) 70%, transparent 100%)",
        }}
        aria-hidden="true"
      />
    </div>
      ...
      <div className="max-w-4xl mx-auto text-center flex flex-col items-center space-y-8 animate-fade-in">
        <h1
          ...
          style={{
            ...
            textShadow: "0 1px 24px hsl(var(--background) / 0.85)",
          }}
        >
          Houston's Medical &amp;{" "}
        ...
        <p
          className="text-lg md:text-xl font-body leading-relaxed max-w-2xl text-foreground/80"
          style={{ textShadow: "0 1px 16px hsl(var(--background) / 0.9)" }}
        >
          Dr. Ahmad Rehmani, D.O. delivers elite, fellowship-trained care for complex
          retinal and ocular conditions at{" "}
          <span className="text-foreground font-semibold">North Houston Retina</span>.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 w-full">
          ...
          <Link
            to="/services"
            className="inline-flex items-center justify-center px-8 py-4 bg-background/90 hover:bg-background border border-border backdrop-blur-sm transition-all text-foreground font-body font-semibold rounded-lg w-full sm:w-auto shadow-md"
          >
